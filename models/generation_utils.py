...
class GreedySearchDecoderOnlyOutput(ModelOutput):
    ...
    qst_past_key_values: Optional[Tuple[Tuple[Tuple[torch.FloatTensor]]]] = None
...
class GenerationMixin:
    ...
    def _extract_qst_past_from_model_output(self, outputs: ModelOutput, standardize_cache_format: bool = False):
        past_key_values = None
        if "qst_past_key_values" in outputs:
            past_key_values = outputs.qst_past_key_values
        elif "mems" in outputs:
            past_key_values = outputs.mems
        elif "past_buckets_states" in outputs:
            past_key_values = outputs.past_buckets_states
    ...
    def _update_model_kwargs_for_generation(
        ...
        model_kwargs["qst_past_key_values"] = self._extract_qst_past_from_model_output(
            outputs, standardize_cache_format=standardize_cache_format
        )
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # print(model_kwargs['past_key_values'][0][0].size(), model_kwargs['qst_past_key_values'][0][1].size())
            # exit()
            # if eos_token was found in one sentence, set sentence to finished
            if eos_token_id_tensor is not None:
                unfinished_sequences = unfinished_sequences.mul(
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return GreedySearchEncoderDecoderOutput(
                    ...
                    decoder_hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                    qst_past_key_values=model_kwargs.get("qst_past_key_values"),
                )
            else:
