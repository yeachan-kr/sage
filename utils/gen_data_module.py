...
def make_data_module(input_tokenizer: transformers.PreTrainedTokenizer, output_tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        if dataset_name == 'alpaca':
            ...
            def multiple_choice_format_arc_e(example):
                question = example['question']
                choices = example['choices']['text']
                choices_label = example['choices']['label']
                return {
                    'input': f'{question}\n\n' + '\n '.join([f"{i}) {choice}" for i, choice in zip(choices_label, choices)]),
                    'output': example['answerKey']
                }
            dataset = dataset.map(multiple_choice_format_arc_e)
            return dataset
        elif dataset_name == 'piqa':
            dataset = load_dataset('ybisk/piqa')
            dataset = dataset.map(lambda x: {'input': f'Question: {x["goal"]} \n Sol1) {x["sol1"]} \n Sol2) {x["sol2"]} \n Answer: Sol', 'output': cls_label_map[dataset_name][x['label']]})

            # dataset = dataset.map(lambda x: {'input': f'Goal: {x["goal"]} \n\n \
            #                                  Option 1: {x["sol1"]}\n \
            #                                  Option 2: {x["sol2"]}\n\n \
            #                                  Question: Which option is more plausible?\n \
            #                                  Answer (1 or 2):',
            #                                  'output': cls_label_map[dataset_name][x['label']]})
            return dataset
        elif dataset_name == 'samsum':
            dataset = load_dataset('knkarthick/samsum')
            dataset = dataset.map(lambda x: {'input': f'{x["dialogue"]} \n Summary:', 'output': x['summary']})
            return dataset
        else:
    ...
    def format_dataset(dataset, dataset_format):
        if (
                dataset_format == 'alpaca' or dataset_format == 'alpaca-clean' or dataset_format == 'alpaca-gpt4'
...
class DataCollatorForCausalLM(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        data_dict = {
            'input_ids': input_ids,
            'attention_mask': input_ids.ne(self.input_tokenizer.pad_token_id).float(),
        }
